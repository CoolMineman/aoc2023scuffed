...
public class Day052 {
    ...
    public static void main(String[] args) throws Throwable {
        // try (BufferedReader r = Files.newBufferedReader(Paths.get("testa.txt"))) {
    ...
    /**
     static * Range
long base, long length  implements Comperable    */
    public static record Range(long base, long length) implements Comparable<Range> {

        @Override
        public int compareTo(Range o) {
            return Long.compare(base, o.base);
        }
    }
    ...
    public static void part1() throws Throwable {
        List<Long> seeds = Arrays.stream("""
            1367444651 99920667 3319921504 153335682 67832336 139859832 2322838536 666063790 1591621692 111959634 442852010 119609663 733590868 56288233 2035874278 85269124 4145746192 55841637 864476811 347179760
                    """.strip().split(" ")).map(Long::parseLong).collect(Collectors.toList());
        ArrayList<Range> seedRanges = new ArrayList<>();
        for (int i = 0; i < seeds.size(); i += 2) {
            seedRanges.add(new Range(seeds.get(i), seeds.get(i + 1)));
        }
        HashSet<Range> a = new HashSet<>(seedRanges);
        HashSet<Range> b = new HashSet<>();
        for (String map : new String[] {
            ...
            a.stream().sorted().forEach(System.out::print); System.out.print("\n");
            for (Range ai : a) {
                b.addAll(parse(map, ai));
            }
            a = b;
            b = new HashSet<>();
        }
        a.stream().sorted().forEach(System.out::print); System.out.print("\n");
        System.out.println("Part 1: " + a.stream().sorted().findFirst().get());
    }

    private static List<Range> parse(String in, Range inRange) {
        ArrayList<Range> r = new ArrayList<>();
        ArrayList<Range> usedRanges = new ArrayList<>();
        for (String l : in.strip().split("\n")) {
            if (l.isBlank()) continue;
            String[] yeet = l.strip().split("\\s+");
            if (yeet[0].isBlank()) System.out.println(l);
            long dst = Long.parseLong(yeet[0]);
            long src = Long.parseLong(yeet[1]);
            long range = Long.parseLong(yeet[2]);
            if (inRange.base + inRange.length - 1 >= src && inRange.base < src + range) {
                long start = Math.max(inRange.base, src);
                long end = Math.min(inRange.base + inRange.length, src + range);
                Range srcRange = new Range(start, end - start);
                Range dstRange = new Range(start - src + dst, srcRange.length);
                usedRanges.add(srcRange);
                r.add(dstRange);
            }
        }
        usedRanges.sort(Range::compareTo);
        int curRange = 0;
        for (long i = inRange.base; i < inRange.base + inRange.length;) {
            Range range = null;
            if (curRange < usedRanges.size()) {
                range = usedRanges.get(curRange++);
            }
            if (range == null) {
                r.add(new Range(i, inRange.base + inRange.length - i));
                break;
            } else if (range.base > i) {
                r.add(new Range(i, range.base - i));
            }
            i = range.base + range.length;
        }
        return r;
    }
